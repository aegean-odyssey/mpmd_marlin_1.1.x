 ...
 *
 * Configuration.h (standard Marlin operation procedure)
 */
...
#ifndef MAKE_PROJECT
#define MAKE_PROJECT  "mpmd_marlin_1.1.x"
#endif

#ifndef MAKE_VERSION
#define MAKE_VERSION  "119"
#endif

#ifndef MAKE_RELEASE
#define MAKE_RELEASE  "00"
#endif
...
#define STRING_CONFIG_H_AUTHOR  BUILD_AUTHOR     // who made the changes
#define SHOW_BOOTSCREEN
#define STRING_SPLASH_LINE1 SHORT_BUILD_VERSION  // shown during bootup, line 1
#define STRING_SPLASH_LINE2 WEBSITE_URL          // shown during bootup, line 2
 ...
 * default port pins. Serial port 0 is always used by the Arduino bootloader 
 ...
 *
 * 250000 works in most cases, but you might try a lower speed if you 
 * commonly experience drop-outs during host printing. You may try up 
 * to 1000000 to speed up SD file transfer.
 *
...
// programs to differentiate between machines). You can use an online service
// to generate a random UUID. (eg http://www.uuidgenerator.net/version4)
 ...
 *   - Extends the stepping routines to move multiple steppers in proportion 
 *     to the mix.
...
// on firmware to position when changing). The offset has to be X=0, Y=0
// for the extruder 0 hotend (default extruder).
 ...
 * 0 = No Power Switch
 * 1 = ATX
 * 2 = X-Box 360 203Watts (blue wire connected to PS_ON; red wire to VCC)
 *
 ...
 *     4 : 10k thermistor !! do not use it for a hotend. It gives bad
 *         resolution at high temp. !!
 *     5 : 100K thermistor - ATC Semitec 104GT-2/104NT-4-R025H42G (Used
 *         in ParCan & J-Head) (4.7k pullup)
 ...
 *    12 : 100k 0603 SMD Vishay NTCS0603E3104FXT (4.7k pullup) (calibrated 
 *         for Makibox hot bed)
 *    13 : 100k Hisens 3950  1% up to 300°C for hotend "Simple ONE " & 
 *         "Hotend "All In ONE"
 ...
 *         thermistor
 *
 *       1k ohm pullup tables - This is atypical and requires changing out the
 *       4.7k pullup for 1k. (but gives greater accuracy and more stable PID)
 *
 *    51 : 100k thermistor - EPCOS (1k pullup)
 *    52 : 200k thermistor - ATC Semitec 204GT-2 (1k pullup)
 *    55 : 100k thermistor - ATC Semitec 104GT-2 (Used in ParCan & J-Head)
 *         (1k pullup)
 *
 ...
 *
 *       Use these for Testing or Development purposes. 
 *       NEVER for production machine.
 *
...
// Extruder temperature must be close to target for this long before
// M109 returns success
#define TEMP_RESIDENCY_TIME  10  // (seconds)
// (degC) range of +/- temperatures considered "close" to the target one
#define TEMP_HYSTERESIS  3
// (degC) Window around target to start the residency timer x degC early.
#define TEMP_WINDOW  1

// Bed temperature must be close to target for this long before
// M190 returns success
...
// will not be enabled. It is used to check that the wiring to the thermistor
// is not broken. Otherwise this would lead to the heater being powered on
// all of the time.
...
#define BANG_MAX  255
// Limits current to nozzle while PID is active (see
// PID_FUNCTIONAL_RANGE below); 255=full current
#define PID_MAX  BANG_MAX
// Smoothing factor within any PID loop
#define PID_K1  0.95
#if ENABLED(PIDTEMP)
  // Add PID Autotune to the LCD "Temperature" menu to run M303 and
  // apply the result.
  ...
  // PWM with very low frequency (roughly 0.125Hz=8s) and minimum state time
  // of approximately 1s useful for heaters driven by a relay
  //#define SLOW_PWM_HEATERS
  // Uses separate PID parameters for each extruder (useful for mismatched
  // extruders). Set/get with gcode: M301 E[extruder number, 0-2]
  //#define PID_PARAMS_PER_HOTEND
  // If the temperature difference between the target temperature and
  // the actual temperature is more than PID_FUNCTIONAL_RANGE then the
 ...
 * means 7.689Hz, which is fine for driving a square wave into a resistive 
 * load and does not significantly impact FET heating. This also works fine
 * on a Fotek SSR-10DA Solid State Relay into a 250W heater. 
 * If your configuration is significantly different than this and you don't
 * understand the issues involved, don't use bed PID until someone else 
 * verifies that your hardware works.
 */
#define PIDTEMPBED
 ...
 * hysteresis). When set to any value below 255, enables a form of PWM to
 * the bed that acts like a divider so don't use it unless you are OK with
 * PWM on your bed. (See the comment on enabling PIDTEMPBED)
 */
 ...
 * damage and fire. Marlin always includes safe min and max temperature 
 * ranges which protect against a broken or disconnected thermistor wire.
 *
...
// kinematics either in the usual order or reversed
//#define COREXY
//#define COREXZ
//#define COREYZ
//#define COREYX
...
#define DELTA
  ...
  // uncomment to add G33 Delta Auto-Calibration (Enable EEPROM_SETTINGS
  // to store results)
  #define DELTA_AUTO_CALIBRATION

  // NOTE NB all values for DELTA_* values MUST be floating point, so
  // always have a decimal point in them
    ...
    // Set the radius for the calibration probe points
    // - max DELTA_PRINTABLE_RADIUS for non-eccentric probes
  ...
  #define DELTA_PRINTABLE_RADIUS  55.0 // mm
  ...
  #define DELTA_RADIUS  63.35 //mm

  // Trim adjustments for individual towers
  // tower angle corrections for X and Y tower / rotate XYZ so Z tower
  // angle = 0 measured in degrees anticlockwise looking from above the
  // printer get these values from auto calibrate
...
// or probe. Almost all printers will be using one per axis. Probes will use
// one or more of the extra connectors. Leave undefined any used for
// non-endstop and non-probe purposes.
...
// set to true to invert the logic of the (particular) endstop.
 ...
 * advanced options for stepper drivers that support them. You may also
 * override timing options in Configuration_adv.h.
 *
...
// Enable this feature if all enabled endstop pins are interrupt-capable. This
// will remove the need to poll the interrupt pins, saving many CPU cycles.
 ...
 * essentially noise-proof without sacrificing accuracy. This option also 
 * increases MCU load when endstops or the probe are enabled. So this is NOT
 * recommended. USE AT YOUR OWN RISK. (This feature is not required for common
 * micro-switches mounted on PCBs  based on the Makerbot design, since they
 * already include the 100nF capacitor.)
 */
 ...
 * Override with M92  X, Y, Z, E0 [, E1[, E2[, E3[, E4]]]]
 */
...
// calculate a separate value the extruder
#define DEFAULT_E_STEPS_PER_UNIT ((E_FULL_STEPS_PER_ROTATION * E_MICROSTEPS) / double(2.0 * PI * E_EFFECTIVE_RADIUS))

#define DEFAULT_AXIS_STEPS_PER_UNIT   { DEFAULT_XYZ_STEPS_PER_UNIT, DEFAULT_XYZ_STEPS_PER_UNIT, DEFAULT_XYZ_STEPS_PER_UNIT, DEFAULT_E_STEPS_PER_UNIT }

/**
 * Default Max Feed Rate (mm/s)
 * Override with M203  X, Y, Z, E0 [, E1[, E2[, E3[, E4]]]]
 */
 ...
 * a probe. Use G29 repeatedly, adjusting the Z height at each point with
 * movement commands or (with LCD_BED_LEVELING) the LCD controller.
 */
...
//#define PROBING_HEATERS_OFF
  ...
  // Wait for bed to heat back up between probes (to improve accuracy)
  //#define WAIT_FOR_BED_HEATER
#endif
// Turn fans off when probing
//#define PROBING_FANS_OFF
// To prevent vibrations from triggering piezo sensors
//#define DELAY_BEFORE_PROBING 200  // (ms)
...
//#define Z_PROBE_SLED
// The extra distance the X axis must travel to pickup the sled.
 ...
 * - http://reprap.org/wiki/Kossel#Automatic_bed_leveling_probe
...
//#define Z_PROBE_ALLEN_KEY
  ...
  // loaded touch probe on G29, if servo actuated touch probe is not defined.
...
// For Inverting Stepper Enable Pins (Active Low) use 0,
...
#define DISABLE_X false // DELTA does not invert
...
// Keep only the active extruder enabled.
...
// configure directions via INVERT_STEPPER_DIRECTION_XYZE (see HAL_stm32.h)
// 
#if INVERT_STEPPER_DIRECTION_XYZE & 8
#undef  INVERT_X_DIR
...
// Don't raise Z (lower the bed) if Z is "unknown." For beds that
// fall when Z is powered off.
//#define UNKNOWN_Z_NO_RAISE

// Minimal z height before homing (G28) for Z clearance
// above the bed, clamps, ...
// Be sure you have this distance over your Z_MAX_POS in case.
//#define Z_HOMING_HEIGHT  8  // (mm)
 ...
 *
 * RAMPS-based boards use SERVO3_PIN for the first runout sensor. For other
 * boards you may need to define FIL_RUNOUT_PIN,  FIL_RUNOUT2_PIN, etc. By
 * default the firmware assumes HIGH=FILAMENT PRESENT.
 */
...
//#define DEBUG_LEVELING_FEATURE
  ...
  // Length of all segments (except the last one)
  #define LEVELED_SEGMENT_LENGTH 5.0 // (mm)
  ...
  //#define G26_MESH_VALIDATION
    ...
    #define MESH_TEST_HOTEND_TEMP  205.0  // (°C)
    // Default bed temperature for the G26 Mesh Validation Tool.
    #define MESH_TEST_BED_TEMP      60.0  // (°C)
  #endif
  ...
  // When the nozzle is off the mesh, this value is used
...
//#define LCD_BED_LEVELING
  ...
  // Z Range centered on Z_MIN_POS for LCD Z adjustment
...
//#define LEVEL_BED_CORNERS
  ...
  // An inset for corner leveling
  #define LEVEL_CORNERS_INSET  30 // (mm)
  // Move nozzle up before moving between corners
  #define LEVEL_CORNERS_Z_HOP  4.0 // (mm)
...
// Manually set the home position. Leave undefined for automatic settings.
...
//   enabled.
// - If stepper drivers time out, it will need X and Y homing again
//   before Z homing.
...
#define Z_SAFE_HOMING
  ...
  // X point for Z homing when homing all axes (G28).
  #define Z_SAFE_HOMING_X_POINT ((X_BED_SIZE) / 2)
  // Y point for Z homing when homing all axes (G28).
...
//
// Enable for M500 and M501 commands
#define EEPROM_SETTINGS
// Saves ~2700 bytes of PROGMEM. Disable for release!
//#define DISABLE_M503
// Give feedback on EEPROM commands. Disable to save PROGMEM.
...
//
// Add M100 (Free Memory Watcher) to debug memory usage
...
#define NOZZLE_PARK_FEATURE
  ...
  // X and Y axes feedrate in mm/s (also used for delta printers Z axis)
...
//
// Add individual axis homing items (Home X, Home Y, and Home Z) to
// the LCD menu.
...
//
// Original Ulticontroller from Ultimaker 2 printer with
// SSD1309 I2C display and encoder.
...
// heaters, and the fan if FAN_SOFT_PWM is enabled. However, control
// resolution will be halved for each increment; at zero value, there
// are 128 effective control positions.
#define SOFT_PWM_SCALE 0

// If SOFT_PWM_SCALE is set to a value higher than 0, dithering can be used
// to mitigate the associated resolution loss. If enabled, some of the PWM
// cycles are stretched so on average the desired duty cycle is attained.
 ...
 *
 * Enable support for an RGB LED connected to 5V digital pins, or  an RGB
 * Strip connected to MOSFETs controlled by digital pins.
 *
 * Adds the M150 command to set the LED (or LED strip) color. If pins are
 * PWM capable (e.g., 4, 5, 6, 11) then a range of  luminance values can be
 * set from 0 to 255. For Neopixel LED an overall brightness parameter is
 * also available.
 *
 * *** CAUTION ***
 *  LED Strips require a MOSFET Chip between PWM lines and LEDs, as the
 *  Arduino cannot handle the current the LEDs will require. Failure to
 *  follow this precaution can destroy your Arduino!
...
//#define NEOPIXEL_LED
  ...
  // LED driving pin on motherboard
  // 4 => D4 (EXP2-5 on Printrboard); 30 => PC7 (EXP3-13 on Rumba)
 ...
 *
 *  - Gradually change from blue to violet as the heated bed reaches 
...
// Delay (in milliseconds) before the next move will start, to give the
// servo time to reach its target angle. 300ms is a good value but you
// can try less delay. If the servo can't reach the requested position,
// increase it.
