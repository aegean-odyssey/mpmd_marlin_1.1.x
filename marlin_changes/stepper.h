...
// ###AO### *!* backport from Marlin 2.0, use 32-bit calculation
      ...
      if (step_rate >= (8 * 256)) { // higher step rate
        ...
        const PGM_PATCH_uint16_t table_address = (PGM_PATCH_uint16_t)&speed_lookuptable_fast[(uint8_t)(step_rate >> 8)][0],
                       gain = (uint16_t)pgm_read_word_near(table_address + 2);
        timer = MultiU16X8toH16(tmp_step_rate, gain);
        timer = (uint16_t)pgm_read_word_near(table_address) - timer;
      }
      else { // lower step rates
        PGM_PATCH_uint16_t table_address = (PGM_PATCH_uint16_t)&speed_lookuptable_slow[0][0];
