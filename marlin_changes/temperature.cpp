...
static void* heater_ttbl_map[2] =
    { (void *) HEATER_0_TEMPTABLE,
      (void *) HEATER_1_TEMPTABLE };
static constexpr uint8_t heater_ttbllen_map[2] =
    { HEATER_0_TEMPTABLE_LEN,
      HEATER_1_TEMPTABLE_LEN };
#else
static void* heater_ttbl_map[HOTENDS] =
    ARRAY_BY_HOTENDS((void *) HEATER_0_TEMPTABLE,
		     (void *) HEATER_1_TEMPTABLE,
		     (void *) HEATER_2_TEMPTABLE,
		     (void *) HEATER_3_TEMPTABLE,
		     (void *) HEATER_4_TEMPTABLE);
static constexpr uint8_t heater_ttbllen_map[HOTENDS] =
    ARRAY_BY_HOTENDS(HEATER_0_TEMPTABLE_LEN,
		     HEATER_1_TEMPTABLE_LEN,
...
float Temperature::Ki[HOTENDS];
float Temperature::Kd[HOTENDS];
#if ENABLED(PID_EXTRUSION_SCALING)
float Temperature::Kc[HOTENDS];
#endif
#else
float Temperature::Kp;
float Temperature::Ki;
float Temperature::Kd;
#if ENABLED(PID_EXTRUSION_SCALING)
...
// Init min and max temp with extreme values to prevent
// false errors during startup
/* ###AO### FIXME! need a different approach for patchable tables */
int16_t Temperature::minttemp_raw[HOTENDS] =
    ARRAY_BY_HOTENDS(HEATER_0_RAW_LO_TEMP,
		     HEATER_1_RAW_LO_TEMP,
		     HEATER_2_RAW_LO_TEMP,
		     HEATER_3_RAW_LO_TEMP,
		     HEATER_4_RAW_LO_TEMP);
int16_t Temperature::maxttemp_raw[HOTENDS] =
    ARRAY_BY_HOTENDS(HEATER_0_RAW_HI_TEMP,
		     HEATER_1_RAW_HI_TEMP,
		     HEATER_2_RAW_HI_TEMP,
		     HEATER_3_RAW_HI_TEMP,
		     HEATER_4_RAW_HI_TEMP);

int16_t Temperature::minttemp[HOTENDS] = { 0 };
int16_t Temperature::maxttemp[HOTENDS] = ARRAY_BY_HOTENDS1(16383);
...
uint8_t Temperature::soft_pwm_count_fan[FAN_COUNT];
#endif

#if ENABLED(FILAMENT_WIDTH_SENSOR)
// measured filament diameter - one extruder only
 ...
 */
void Temperature::pid_autotune(const float &target,
			       const int8_t hotend,
			       ...
			       const bool set_result/*=false*/)
{
    ...
    millis_t t1 = next_temp_ms;
    millis_t t2 = next_temp_ms;
...
#define HAS_TP_BED (ENABLED(THERMAL_PROTECTION_BED) && ENABLED(PIDTEMPBED))
    ...
    const float watch_temp_target = target + 1 - float(watch_temp_increase)
	- (hotend < 0) ? TEMP_BED_HYSTERESIS : TEMP_HYSTERESIS;
#elif HAS_TP_BED
    const uint16_t watch_temp_period = WATCH_BED_TEMP_PERIOD;
    const uint8_t watch_temp_increase = WATCH_BED_TEMP_INCREASE;
    const float watch_temp_target = target
	- float(watch_temp_increase + TEMP_BED_HYSTERESIS) + 1;
#else
    const uint16_t watch_temp_period = WATCH_TEMP_PERIOD;
    const uint8_t watch_temp_increase = WATCH_TEMP_INCREASE;
    const float watch_temp_target = target
	- float(watch_temp_increase + TEMP_HYSTERESIS) + 1;
#endif
    ...
    if (!WITHIN(hotend,
		(ENABLED(PIDTEMPBED) ? -1 : 0),
		(ENABLED(PIDTEMP) ? (HOTENDS - 1) : -1))) {
	SERIAL_ECHOLNPGM(MSG_PID_BAD_EXTRUDER_NUM);
	return;
    }

#if HAS_PID_FOR_BOTH
    if (target > (((hotend < 0) ? BED_MAXTEMP : maxttemp[hotend]) - 15)) {
	SERIAL_ECHOLNPGM(MSG_PID_TEMP_TOO_HIGH);
	return;
    }
#elif ENABLED(PIDTEMPBED)
    if (target > (BED_MAXTEMP - 15)) {
	SERIAL_ECHOLNPGM(MSG_PID_TEMP_TOO_HIGH);
	return;
    }
#else
    if (target > (maxttemp[hotend] - 15)) {
	SERIAL_ECHOLNPGM(MSG_PID_TEMP_TOO_HIGH);
	return;
    }
#endif
...
#if HAS_PID_FOR_BOTH
    if (hotend < 0)
	soft_pwm_amount_bed = bias;
    else
	soft_pwm_amount[hotend] = bias;
#elif ENABLED(PIDTEMPBED)
    soft_pwm_amount_bed = bias;
#else
    soft_pwm_amount[hotend] = bias;
	...
	if (temp_meas_ready) { // temp sample ready
...
#elif ENABLED(PIDTEMPBED)
	    current = current_temperature_bed;
#else
	    current = current_temperature[hotend];
#endif
	    NOLESS(max, current);
	    NOMORE(min, current);
		...
		if (ELAPSED(ms, t1 + 5000UL)) {
		    ...
		    if (cycles > 0) {
#if HAS_PID_FOR_BOTH
			const long max_pow = (hotend < 0)
			    ? MAX_BED_POWER
			    : PID_MAX;
#elif ENABLED(PIDTEMPBED)
			const long max_pow = MAX_BED_POWER;
#else
			const long max_pow = PID_MAX;
#endif
...
#define MAX_OVERSHOOT_PID_AUTOTUNE 20
	...
	if (ELAPSED(ms, next_temp_ms)) {
		...
		if (!heated) {
		    ...
		    if (current > next_watch_temp) {
			// Over the watch temp?
			// - set the next temp to watch for
			// - move the expiration timer up
			// - Flag if target temperature reached
			next_watch_temp = current + watch_temp_increase;
			temp_change_ms = ms + watch_temp_period * 1000UL;
			if (current > watch_temp_target) heated = true;
		    }
		    else if (ELAPSED(ms, temp_change_ms))
			// Watch timer expired
			_temp_error(hotend, PSTR(MSG_T_HEATING_FAILED),
				    TEMP_ERR_PSTR(MSG_HEATING_FAILED_LCD,
						  hotend));
		  }
		else if (current < target - (MAX_OVERSHOOT_PID_AUTOTUNE))
		    // Heated, then temperature fell too far?
		    _temp_error(hotend, PSTR(MSG_T_THERMAL_RUNAWAY),
				TEMP_ERR_PSTR(MSG_THERMAL_RUNAWAY, hotend));
	    }
#endif
	} // every 2 seconds

	// timeout after MAX_CYCLE_TIME_PID_AUTOTUNE minutes
	// since the last undershoot/overshoot cycle
#ifndef MAX_CYCLE_TIME_PID_AUTOTUNE
#define MAX_CYCLE_TIME_PID_AUTOTUNE 20L
#endif
	if (((ms - t1) + (ms - t2)) >
	    (MAX_CYCLE_TIME_PID_AUTOTUNE * 60L * 1000L)) {
	...
	if (cycles > ncycles) {
	    ...
	    SERIAL_PROTOCOLPAIR("#define DEFAULT_", estring);
	    SERIAL_PROTOCOLPAIR("Kp ", workKp);
	    SERIAL_EOL();
	    SERIAL_PROTOCOLPAIR("#define DEFAULT_", estring);
	    SERIAL_PROTOCOLPAIR("Ki ", workKi);
...
void Temperature::check_extruder_auto_fans()
{
    static const pin_t fanPin[] PROGMEM =
	{ E0_AUTO_FAN_PIN,
	  E1_AUTO_FAN_PIN,
	  E2_AUTO_FAN_PIN,
	  E3_AUTO_FAN_PIN,
	...
	{ 0,
	  ...
	  AUTO_4_IS_0 ? 0 : AUTO_4_IS_1 ? 1 : AUTO_4_IS_2
	  ? 2 : AUTO_4_IS_3 ? 3 : 4,
	  AUTO_CHAMBER_IS_0 ? 0 : AUTO_CHAMBER_IS_1 ? 1 : AUTO_CHAMBER_IS_2
	  ? 2 : AUTO_CHAMBER_IS_3 ? 3 : AUTO_CHAMBER_IS_4 ? 4 : 5
	};
    uint8_t fanState = 0;
...
#ifdef ARDUINO
	const pin_t pin = pgm_read_byte(&fanPin[f]);
#else
	const pin_t pin = fanPin[f];
#endif
	const uint8_t bit = pgm_read_byte(&fanBit[f]);
	if (pin >= 0 && !TEST(fanDone, bit)) {
	    uint8_t newFanSpeed = TEST(fanState, bit)
		? EXTRUDER_AUTO_FAN_SPEED : 0;
#if ENABLED(AUTO_POWER_E_FANS)
	    autofan_speed[f] = newFanSpeed;
#endif
	    // this idiom allows both digital and PWM
	    // fan outputs (see M42 handling).
...
//
void Temperature::_temp_error(const int8_t e, const char * const serial_msg,
			      const char * const lcd_msg)
{
    if (IsRunning()) {
	SERIAL_ERROR_START();
...
#define _HOTEND_TEST     e == active_extruder
    ...
    pid_error[HOTEND_INDEX] = target_temperature[HOTEND_INDEX]
	- current_temperature[HOTEND_INDEX];
    dTerm[HOTEND_INDEX] = PID_K2 * PID_PARAM(Kd, HOTEND_INDEX)
	* (current_temperature[HOTEND_INDEX] - temp_dState[HOTEND_INDEX])
	+ float(PID_K1) * dTerm[HOTEND_INDEX];
	...
	|| heater_idle_timeout_exceeded[HOTEND_INDEX]
        ...
        pTerm[HOTEND_INDEX] = PID_PARAM(Kp, HOTEND_INDEX)
	    * pid_error[HOTEND_INDEX];
        temp_iState[HOTEND_INDEX] += pid_error[HOTEND_INDEX];
        iTerm[HOTEND_INDEX] = PID_PARAM(Ki, HOTEND_INDEX)
	    * temp_iState[HOTEND_INDEX];

        pid_output = pTerm[HOTEND_INDEX] + iTerm[HOTEND_INDEX]
	    - dTerm[HOTEND_INDEX];
	...
	if (_HOTEND_TEST) {
		...
		lpq[lpq_ptr] = 0;
            ...
            cTerm[HOTEND_INDEX] = (lpq[lpq_ptr] * planner.steps_to_mm[E_AXIS])
		* PID_PARAM(Kc, HOTEND_INDEX);
...
#endif
	pid_output = (current_temperature[HOTEND_INDEX] <
		      target_temperature[HOTEND_INDEX]) ? PID_MAX : 0;
#endif
    return pid_output;
}
...
float Temperature::get_pid_output_bed()
{
    ...
    dTerm_bed = PID_K2 * bedKd * (current_temperature_bed - temp_dState_bed)
	+ PID_K1 * dTerm_bed;
...
void Temperature::manage_heater()
{
...
#if ENABLED(HEATER_0_USES_MAX6675)
    if (current_temperature[0] > MIN(HEATER_0_MAXTEMP, MAX6675_TMAX - 1.0))
	max_temp_error(0);
    if (current_temperature[0] < MAX(HEATER_0_MINTEMP, MAX6675_TMIN + .01))
	min_temp_error(0);
  #endif

#if WATCH_HOTENDS || WATCH_THE_BED || DISABLED(PIDTEMPBED) \
    || HAS_AUTO_FAN || HEATER_IDLE_HANDLER
    millis_t ms = millis();
#endif

    HOTEND_LOOP() {
#if HEATER_IDLE_HANDLER
	if (!heater_idle_timeout_exceeded[e]
	    && heater_idle_timeout_ms[e]
	    && ELAPSED(ms, heater_idle_timeout_ms[e]))
	...
	soft_pwm_amount[e] = (((current_temperature[e] > minttemp[e])
			       || is_preheating(e))
			      && ((current_temperature[e] < maxttemp[e])
				  ? ((int)get_pid_output(e) >> 1) : 0));

#if WATCH_HOTENDS
	// Make sure temperature is increasing
	if (watch_heater_next_ms[e] && ELAPSED(ms, watch_heater_next_ms[e])) {
	    // Time to check this extruder?
	    if (degHotend(e) < watch_target_temp[e])
		// Failed to increase enough?
		_temp_error(e, PSTR(MSG_T_HEATING_FAILED),
			    TEMP_ERR_PSTR(MSG_HEATING_FAILED_LCD, e));
	    else
	...
	if (ABS(current_temperature[0] - redundant_temperature)
	    > MAX_REDUNDANT_TEMP_SENSOR_DIFF)
	    _temp_error(0, PSTR(MSG_REDUNDANCY),
			PSTR(MSG_ERR_REDUNDANT_TEMP));
#endif
    } // HOTEND_LOOP
	...
	if (meas_shift_index < 0)
	    ...
	    meas_shift_index += MAX_MEASUREMENT_DELAY + 1;
	meas_shift_index = constrain(meas_shift_index, 0,
    ...
    if (watch_bed_next_ms && ELAPSED(ms, watch_bed_next_ms)) {
	    ...
	    // Failed to increase enough?
	    _temp_error(-1, PSTR(MSG_T_HEATING_FAILED),
			TEMP_ERR_PSTR(MSG_HEATING_FAILED_LCD, -1));
        else
	    // Start again if the target is still far off
	    start_watching_bed();
    }
...
#if HEATER_IDLE_HANDLER
    if (!bed_idle_timeout_exceeded && bed_idle_timeout_ms
	&& ELAPSED(ms, bed_idle_timeout_ms))
...
#if ENABLED(PIDTEMPBED)
	    soft_pwm_amount_bed =
		WITHIN(current_temperature_bed, BED_MINTEMP, BED_MAXTEMP)
...
static float TEMP_MAX6675(int16_t r)
{
    return r * 0.25;
}

static float TEMP_AD595(int16_t r)
{
    return r *5.0 *100.0/1024.0 /(OVERSAMPLENR) *(TEMP_SENSOR_AD595_GAIN)
	+ TEMP_SENSOR_AD595_OFFSET;
}

static float TEMP_AD8495(int16_t r)
{
    return r *6.6 *100.0 /1024.0 /(OVERSAMPLENR) *(TEMP_SENSOR_AD8495_GAIN)
	+ TEMP_SENSOR_AD8495_OFFSET;
}

/* ###AO### */
#if MB(MALYAN_M300)
/* Rework the thermistor table lookup so that it's a proper function
(i.e. NOT code buried in some preprocessor macro) AND to prepare for new
thermistor tables that pick up a few extra significant digits by scaling
the integer temperature values (x100). Current Marlin thermistor tables
will need to be scaled appropriately. */

typedef struct {
    const short r, t;
} tce_t;

#define TCE(TBL) ((const tce_t *) TBL)

static float TEMP_TABLE(const short x, const short N, const tce_t * table)
{
    //constexpr float TS = 100.0;
    constexpr float TS = 1.0; // for now *!*

    for (short m, a=0, b=N; ;) {
	m = (a + b) >> 1;
	if (m == 0) {
	    // underflow
	    return float(table[0].t)/TS;
	}
	if (m == a) {
	    // overflow
	    return float(table[N-1].t)/TS;
	}
	short xa = table[m-1].r;
	if (x < xa) {
	    b = m;
	    continue;
	}
	short xb = table[m-0].r;
	if (x > xb) {
	    a = m;
	    continue;
	}
	const short ya = table[m-1].t;
	const short yb = table[m-0].t;
	return (ya+(x-xa)*float(yb-ya)/float(xb-xa))/TS;
    }
}
#else  // !MB(MALYAN_M300)
/**
...
/* ###AO### */
#if MB(MALYAN_M300)
    return TEMP_TABLE(raw, heater_ttbllen_map[e], TCE(heater_ttbl_map[e]));
#else
...
/* ###AO### */
#if MB(MALYAN_M300)
    return TEMP_TABLE(raw, BEDTEMPTABLE_LEN, TCE(BEDTEMPTABLE));
#else
...
/* ###AO### */
#if MB(MALYAN_M300)
    return TEMP_TABLE(raw, CHAMBERTEMPTABLE_LEN, TCE(CHAMBERTEMPTABLE));
#else
...
void Temperature::calculate_celsius_temperatures()
{
    ...
    HOTEND_LOOP()
	current_temperature[e] =
	analog_to_celsius_hotend(current_temperature_raw[e], e);
#if HAS_HEATED_BED
    current_temperature_bed =
	analog_to_celsius_bed(current_temperature_bed_raw);
#endif
#if HAS_TEMP_CHAMBER
    current_temperature_chamber =
	analog_to_celsius_chamber(current_temperature_chamber_raw);
#endif
#if ENABLED(TEMP_SENSOR_1_AS_REDUNDANT)
    redundant_temperature =
	analog_to_celsius_hotend(redundant_temperature_raw, 1);
#endif
#if ENABLED(FILAMENT_WIDTH_SENSOR)
...
{
    if (ABS(filament_width_nominal - filament_width_meas)
	<= FILWIDTH_ERROR_MARGIN)
	return int(100.0f * filament_width_nominal
		   / filament_width_meas) - 100;
    return 0;
}
#endif
...
{
#if MB(RUMBA) &&				\
    (ENABLED(HEATER_0_USES_AD595) ||		\
     ENABLED(HEATER_1_USES_AD595) ||		\
     ENABLED(HEATER_2_USES_AD595) ||		\
     ENABLED(HEATER_3_USES_AD595) ||		\
     ENABLED(HEATER_4_USES_AD595) ||		\
     ENABLED(HEATER_BED_USES_AD595) ||		\
     ENABLED(HEATER_CHAMBER_USES_AD595) ||	\
     ENABLED(HEATER_0_USES_AD8495) ||		\
     ENABLED(HEATER_1_USES_AD8495) ||		\
     ENABLED(HEATER_2_USES_AD8495) ||		\
     ENABLED(HEATER_3_USES_AD8495) ||		\
     ENABLED(HEATER_4_USES_AD8495) ||		\
     ENABLED(HEATER_BED_USES_AD8495) ||		\
     ENABLED(HEATER_CHAMBER_USES_AD8495))
    // Disable RUMBA JTAG in case the thermocouple extension
    // is plugged on top of JTAG connector
    MCUCR = _BV(JTD);
    MCUCR = _BV(JTD);
#endif
...
#endif
#endif
#if HAS_AUTO_FAN_4 && !AUTO_4_IS_0 && !AUTO_4_IS_1 && !AUTO_4_IS_2 \
    && !AUTO_4_IS_3
...
#endif
  #endif
#if HAS_AUTO_CHAMBER_FAN && !AUTO_CHAMBER_IS_0 && !AUTO_CHAMBER_IS_1 \
    && !AUTO_CHAMBER_IS_2 && !AUTO_CHAMBER_IS_3 && ! AUTO_CHAMBER_IS_4
#if CHAMBER_AUTO_FAN_PIN == FAN1_PIN
    SET_OUTPUT(CHAMBER_AUTO_FAN_PIN);
#if ENABLED(FAST_PWM_FAN)
    // No prescaling. Pwm frequency = F_CPU/256/8
    setPwmFrequency(CHAMBER_AUTO_FAN_PIN, 1);
#endif
#else
    SET_OUTPUT(CHAMBER_AUTO_FAN_PIN);
#endif
    ...
    // ###AO### FIXME! problem here for patchable tables. The code
    // below tries to find the "raw" a/d steps that correspond to
    // the minimum and maximum temperatures for a particular heater --
    // essentially a "reverse" table lookup. Many of the parameters
    // are established at compile time -- not workable if we hope to
    // be able to "patch" temperature table. More work needed here.
...
void Temperature::start_watching_heater(const uint8_t e)
{
	...
	(degTargetHotend(HOTEND_INDEX)
	 - (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1))) {
	watch_target_temp[HOTEND_INDEX] =
	    degHotend(HOTEND_INDEX) + WATCH_TEMP_INCREASE;
	watch_heater_next_ms[HOTEND_INDEX] =
	    millis() + (WATCH_TEMP_PERIOD) * 1000UL;
    }
    else
...
#if ENABLED(THERMAL_PROTECTION_HOTENDS)
Temperature::TRState Temperature::thermal_runaway_state_machine[HOTENDS] =
    { TRInactive };
millis_t Temperature::thermal_runaway_timer[HOTENDS] = { 0 };
#endif

#if HAS_THERMALLY_PROTECTED_BED
Temperature::TRState Temperature::thermal_runaway_bed_state_machine =
    TRInactive;
    millis_t Temperature::thermal_runaway_bed_timer;
#endif

void Temperature::thermal_runaway_protection(Temperature::TRState* const state,
					     millis_t * const timer,
					     const float &current,
					     const float &target,
					     const int8_t heater_id,
					     ...
					     const uint16_t hysteresis_degc)
{
    ...
    if (heater_id >= 0)
	SERIAL_ECHOPAIR(" ;  Idle Timeout:",
			heater_idle_timeout_exceeded[heater_id]);
    else
	SERIAL_ECHOPAIR(" ;  Idle Timeout:",
			bed_idle_timeout_exceeded);
    SERIAL_EOL();
    */
	...
	break;
    case TRFirstHeating:
	// when first heating, wait for the temperature to be reached
	// then go to, FALLS THROUGH to, Stable state
    ...
    case TRRunaway:
        _temp_error(heater_id, PSTR(MSG_T_THERMAL_RUNAWAY),
		    TEMP_ERR_PSTR(MSG_THERMAL_RUNAWAY, heater_id));
    }
}
...
int Temperature::read_max6675()
{
    ...
    WRITE(MAX6675_SS, 0); // enable
    DELAY_NS(100);        // delay
    // read a big-endian temperature value
    max6675_temp = max6675_spi.receive();
    max6675_temp = (max6675_temp << 8) | max6675_spi.receive();
#if ENABLED(MAX6675_IS_MAX31855)
    max6675_temp = (max6675_temp << 8) | max6675_spi.receive();
    max6675_temp = (max6675_temp << 8) | max6675_spi.receive();
#endif
    WRITE(MAX6675_SS, 1); // disable
    ...
    if (max6675_temp & 0x00002000)
	max6675_temp |= 0xFFFFC000;
#endif
    return (int)max6675_temp;
}
#endif // HEATER_0_USES_MAX6675
...
{
    // Update the raw values if they've been read, otherwise we
    // could be updating them during reading.
    if (! temp_meas_ready)
...
/* ###AO### */
    ...
    /* ###AO### FIXME! won't work with patchable tables */
#define TEMPDIR(N) ((HEATER_##N##_RAW_LO_TEMP) > (HEATER_##N##_RAW_HI_TEMP) ? -1 : 1)
	...
	const int16_t tdir = temp_dir[e];
	const int16_t rawtemp = current_temperature_raw[e] * tdir;
#if ENABLED(PIDTEMP)
	...
	const bool heater_on = (target_temperature[e] > 0);
#endif
	if (rawtemp > maxttemp_raw[e] * tdir)
	    max_temp_error(e);
	if ((rawtemp < (minttemp_raw[e] * tdir)) &&
	    (! is_preheating(e)) && heater_on) {
#ifdef MAX_CONSECUTIVE_LOW_TEMPERATURE_ERROR_ALLOWED
	    if (++consecutive_low_temperature_error[e] >=
    ...
    /* ###AO### FIXME! won't work with patchable tables */
#if HEATER_BED_RAW_LO_TEMP > HEATER_BED_RAW_HI_TEMP
#define GEBED(a,b) ((a) <= (b))
#else
#define GEBED(a,b) ((a) >= (b))
#endif

    if (GEBED(current_temperature_bed_raw, bed_maxttemp_raw))
	max_temp_error(-1);

    if (GEBED(bed_minttemp_raw, current_temperature_bed_raw)) {
	// error only if the bed is actually on
#if ENABLED(PIDTEMPBED)
	if ((target_temperature_bed > 0) || (soft_pwm_amount_bed > 0))
	    min_temp_error(-1);
#else
...
#define ISR_STATICS(n) static uint8_t \
	soft_pwm_count_ ## n = 0
#else
...
#define ISR_STATICS(n) static uint8_t  \
    ...
    constexpr uint8_t pwm_mask = _BV(SOFT_PWM_SCALE) - 1;
#else
...
#else
	// mirror the bed heater pwm. Always start with off to simplify the
	// code that limits current consumption when trying to heat both the
	// hotend and the bed simultaneously (i.e. ONLY_ONE_HEATER_AT_A_TIME).
	// BUT, the trade-off here limits the heat bed "full-on" to 99.2%
	// (127/128).
	WRITE_HEATER_BED(LOW);
#endif
#endif
#if ENABLED(FAN_SOFT_PWM)
#define SPCF soft_pwm_count_fan
#if HAS_FAN0
	SPCF[0] = (SPCF[0] & pwm_mask) + (soft_pwm_amount_fan[0] >> 1);
	WRITE_FAN(soft_pwm_count_fan[0] > pwm_mask ? HIGH : LOW);
#endif
#if HAS_FAN1
	SPCF[1] = (SPCF[1] & pwm_mask) + (soft_pwm_amount_fan[1] >> 1);
	WRITE_FAN1(soft_pwm_count_fan[1] > pwm_mask ? HIGH : LOW);
#endif
#if HAS_FAN2
	SPCF[2] = (SPCF[2] & pwm_mask) + (soft_pwm_amount_fan[2] >> 1);
...
#else
	// mirror bed heater pwm, set high at (128 - pwm_count)
	if (pwm_count_tmp >= (128 - soft_pwm_count_BED))
#if ONLY_ONE_HEATER_AT_A_TIME
	    // but only if hotend control is NOT active
...
#define _SLOW_PWM_ROUTINE(NR, src)					\
    ...
    if (soft_pwm_count_ ##NR > 0) {					\
	    ...
	    if (state_heater_ ##NR == 0)				\
		state_timer_heater_ ##NR = MIN_STATE_TIME;		\
	    state_heater_ ##NR = 1;					\
	    WRITE_HEATER_ ##NR(1);					\
        }								\
    }									\
...
#define ACCUMULATE_ADC(var) do{			  \
    ...
    ADCSensorState next_sensor_state = (adc_sensor_state < SensorsReady)
	? (ADCSensorState)(int(adc_sensor_state) + 1) : StartSampling;

    switch (adc_sensor_state) {
    case SensorsReady: {
	// All sensors have been read. Stay in this state for a few
	// ISRs to save on calls to temp update/checking code below.
	constexpr int8_t extra_loops =
	    MIN_ADC_ISR_LOOPS - (int8_t) SensorsReady;
	static uint8_t delay_count = 0;
	if (extra_loops > 0) {
	    if (delay_count == 0)
		delay_count = extra_loops;  // Init this delay
	    if (--delay_count)
		// While delaying, retain this state (else, next state is 0)
		next_sensor_state = SensorsReady;
	    break;
	}
	// fall-through to start sampling
	adc_sensor_state = StartSampling;
	next_sensor_state = (ADCSensorState) (int(StartSampling) + 1);
    }
    case StartSampling:
	// Start of sampling loops. Do updates/checks.
    ...
    case Measure_FILWIDTH:
        ...
        else if (HAL_READ_ADC() > 102) {
	    // Make sure ADC is reading > 0.5 volts, otherwise don't read.
	    // Subtract 1/128th of the raw_filwidth_value
	    raw_filwidth_value -= raw_filwidth_value >> 7;
	    // Add new ADC reading, scaled by 128
	    raw_filwidth_value += uint32_t(HAL_READ_ADC()) << 7;
        }
	break;
#endif
#if ENABLED(ADC_KEYPAD)
...
#if ENABLED(BABYSTEPPING)
    LOOP_XYZ(axis) {
	// get rid of volatile for performance
...
{
#if HAS_TEMP_HOTEND
    SERIAL_PROTOCOLPAIR(" T:", degHotend(target_extruder));
    SERIAL_PROTOCOLPAIR(" /", degTargetHotend(target_extruder));
#if ENABLED(SHOW_TEMP_ADC_VALUES)
    SERIAL_PROTOCOLPAIR(" (", (int) rawHotendTemp(target_extruder));
    SERIAL_PROTOCOLCHAR(')');
#endif
#endif
#if HAS_HEATED_BED
    SERIAL_PROTOCOLPAIR(" B:", degBed());
    SERIAL_PROTOCOLPAIR(" /", degTargetBed());
#if ENABLED(SHOW_TEMP_ADC_VALUES)
    SERIAL_PROTOCOLPAIR(" (", (int) rawBedTemp());
    SERIAL_PROTOCOLCHAR(')');
#endif
#endif
#if HAS_TEMP_CHAMBER
    SERIAL_PROTOCOLPAIR(" C:", degChamber());
    SERIAL_PROTOCOLPAIR(" /", 0.0);
#if ENABLED(SHOW_TEMP_ADC_VALUES)
    SERIAL_PROTOCOLPAIR(" (", (int) rawChamberTemp());
    SERIAL_PROTOCOLCHAR(')');
#endif
#endif
#if HOTENDS > 1
    HOTEND_LOOP() {
	SERIAL_PROTOCOLPAIR(" T",(int) e);
	SERIAL_PROTOCOLPAIR(":", degHotend(e));
	SERIAL_PROTOCOLPAIR(" /", degTargetHotend(e));
#if ENABLED(SHOW_TEMP_ADC_VALUES)
	SERIAL_PROTOCOLPAIR(" (", (int) rawHotendTemp(e));
	SERIAL_PROTOCOLCHAR(')');
#endif
    }
#endif
